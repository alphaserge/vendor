...
namespace chiffon_back.Code
{
    public class DirectoryHelper
    {
        ...
        public static string ComputeDirectory(string uid)
        {
            string baseDir = Path.Combine(Directory.GetCurrentDirectory(), @"images");
            string dir = uid.Replace("-", "").Insert(6, "\\").Insert(4, "\\").Insert(2, "\\");
            return Path.Combine(baseDir, dir);
        }
        public static string ComputeFilePath(string uid, string extension)
        {
            var dirPath = ComputeDirectory(uid);
